...
export const STREAM_CONVERGENCE_DEFAULTS = {
  speed: 1,
  fidelity: 0.5,
  scale: 1,
  brightness: 1,
...
export function StreamConvergenceBackground({ className = "", ...props }: StreamConvergenceBackgroundProps) {
  ...
  const optionsRef = useRef({ ...STREAM_CONVERGENCE_DEFAULTS, ...props });
  optionsRef.current = { ...STREAM_CONVERGENCE_DEFAULTS, ...props };
  ...
  useEffect(() => {
    ...
    const fragment = compile(gl, gl.FRAGMENT_SHADER, `precision mediump float;\n${STREAM_CONVERGENCE_FRAGMENT_SHADER}`);
    ...
    const resize = () => {
      ...
      // Render at an optimal lightweight resolution factor (0.6x) with bilinear CSS scaling
      const width = Math.max(1, Math.round(bounds.width * 0.6));
      const height = Math.max(1, Math.round(bounds.height * 0.6));
      canvas.width = width;
    ...
    const render = (now: number) => {
      // Cap render rate to smooth 60fps
  ...
  const options = optionsRef.current;
  return (
    <div ref={hostRef} className={`threeui-background stream-convergence${className ? ` ${className}` : ""}`}>
      <canvas
        ref={canvasRef}
        style={{
          width: "100%",
...
export default StreamConvergenceBackground;
